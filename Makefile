...
pdf: ## PDFを生成する (default: ./articles/book.pdf) (出力先変更: make pdf PDF=./hoge.pdf)
	$(DOCKER_COMPOSE) exec review /bin/bash -ci '$(MAKE) pdf -f in_docker.mk PDF=$(PDF) HTML_URL=$(HTML_URL) PAGE_FORMAT=$(PAGE_FORMAT) WAIT_FOR=$(WAIT_FOR)'

browse: html ## ブラウザで表示する (vivliostyle経由)
	$(OPEN) $(HTML_URL)
...
watch:
	watchexec --restart --exts $(WATCHED_EXTS) '$(MAKE) pdf || $(NOTIFY) "setulab: failure"'
